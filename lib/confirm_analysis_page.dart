...
/// Эта страница соответствует confirm_analysis.html
/// Она показывает метрики, извлеченные AI, и позволяет
/// пользователю подтвердить их и (если это первый раз) установить цели.
class ConfirmAnalysisPage extends StatefulWidget {
  /// Данные, полученные от AI из /upload_analysis
  final Map<String, dynamic> initialData;

  /// Флаг, определяющий, это первый анализ пользователя или нет.
...
class _ConfirmAnalysisPageState extends State<ConfirmAnalysisPage> {
  ...
  /// Обработчик нажатия главной кнопки "Сохранить"
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (widget.isFirstAnalysis) {
        // 2.A. Это ПЕРВЫЙ анализ. Запускаем "магию" (визуализацию).
        ...
        // 2.B. Это НЕ первый анализ. Показываем комментарий.
  ...
  Widget build(BuildContext context) {
    // Определяем, какой UI показывать
    if (_isLoading) {
  ...
  /// ЭКРАН 1: ФОРМА ПОДТВЕРЖДЕНИЯ (state 1)
  ...
  /// ЭКРАН 3: РЕЗУЛЬТАТ ВИЗУАЛИЗАЦИИ (state 2a)
  ...
  /// ЭКРАН 4: РЕЗУЛЬТАТ C AI-КОММЕНТАРИЕМ (state 2b)

import 'dart:async';
import 'dart:io';
import 'dart:ui' show ImageFilter; // Для блюра

import 'package:camera/camera.dart';
import 'package:flutter/material.dart';
import 'package:permission_handler/permission_handler.dart';
import 'auth_api.dart';
import 'app_theme.dart'; // Для AppColors, Skeleton и KiloCard
...
class _ScanSheetState extends State<ScanSheet> with WidgetsBindingObserver {
  ...
  bool _isAnalyzing = false; // <-- 1. ДОБАВЬТЕ ЭТУ ПЕРЕМЕННУЮ
  ...
  void initState() {
    ...
    // --- ИЗМЕНЕНИЕ ---
    // НЕ запускаем камеру сразу.
    // _initFuture = _initCamera(); // <-- УДАЛЕНО

    // Запускаем инициализацию камеры ПОСЛЕ того,
    // как BottomSheet ЗАВЕРШИТ анимацию открытия.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Проверяем, что виджет все еще "жив"
      if (mounted) {
        // Запускаем _initCamera() и присваиваем Future
        // в _initFuture, чтобы FutureBuilder его "подхватил".
        setState(() {
  ...
  void dispose() {
    ...
    // --- ИЗМЕНЕНИЕ ---
    // Контроллер УЖЕ МОЖЕТ БЫТЬ null, если закрытие
    // было инициировано через _closeSheet().
    // Это нормально и предотвращает двойной dispose.
    _controller?.dispose();
    // --- КОНЕЦ ИЗМЕНЕНИЯ ---
  ...
  // ... (didChangeAppLifecycleState без изменений) ...
  @override
  ...
  Future<void> _closeSheet() async {
    ...
    // 1. Показываем, что мы заняты (чтобы юзер не нажал 5 раз)
    setState(() {
      _busy = true;
    });

    try {
      // 2. Асинхронно и БЕЗОПАСНО выключаем камеру
      await _controller?.dispose();
    } catch (e) {
      print("Ошибка при dispose() камеры: $e");
    }

    // 3. Обнуляем контроллер, чтобы dispose() его не трогал
    // (на случай, если dispose() все равно вызовется)
    _controller = null;

    // 4. Только теперь, когда камера выключена, закрываем BottomSheet
    if (mounted) {
  ...
  /// (ИЗМЕНЕНО) Прямое сохранение приема пищи
  ...
  /// (ИЗМЕНЕНО) Сброс возвращает к камере
  void _resetScanner() {
    setState(() {
      _frozenPicture = null;
    ...
    // Инициализация камеры не нужна, если _controller еще жив
    if (_controller == null) {
      _initFuture = _initCamera();
    }
  }
        ...
        Padding(
          ...
          child: Row(
            children: [
              IconButton(
                // --- ИЗМЕНЕНИЕ ---
                // Блокируем кнопку, если идет процесс закрытия/открытия
        ...
        Expanded(
          child: FutureBuilder(
            ...
            builder: (context, snapshot) {
              ...
              // --- ИСПОЛЬЗУЕМ ANIMATEDSWITCHER ДЛЯ ПЛАВНОГО ПЕРЕХОДА ---
  ...
  // ... (Метод _buildShutterButton без изменений) ...
...
class _AnalysisResultOverlayState extends State<_AnalysisResultOverlay> with SingleTickerProviderStateMixin {
  ...
  Widget _buildCentralContent() {
    if (widget.isAnalyzing) {
      // --- СКЕЛЕТОН (ВМЕСТО ПРОСТОГО ИНДИКАТОРА) ---
      return KiloCard(
        child: Column(
          children: [
    ...
    if (widget.analysisResult != null) {
      // --- КАРТОЧКА РЕЗУЛЬТАТА (С GPT И ВЫБОРОМ ПРИЕМА ПИЩИ) ---
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  // Используем ToggleButtons для выбора
                  SizedBox(
                    width: double.infinity,
                    child: ToggleButtons(
